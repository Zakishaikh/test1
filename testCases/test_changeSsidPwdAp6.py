...
class Test_TR069_ChangeSsidPwdAp6:
    #ReadConfig_ACS_Tree.setSSID(6)
    ...
    def test_check__SSIDPwd_GUI(self, setup):
        ...
        if acs_set_value == 'PASS':
            ...
            adb_result = commonmethod.connectWifiAndroid11(ssid=GUI_SSID, password=Input_File.SSID_Pwd_AP6[0],
                                                           security='wpa2', r=0)
            print('adb result' + adb_result)

            if GUI_Pwd == Input_File.SSID_Pwd_AP6[0] and Packet_Analysis_Result == 'PASS' and adb_result == 'Connected':
                result = 'PASS'
            else:
                result = 'FAIL'
        else:
