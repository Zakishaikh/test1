...
@pytest.mark.order(9)
class Test_TR069_ChangeSsidPwdAp3:
    #ReadConfig_ACS_Tree.setSSID(3)
    ...
    def test_check__SSIDPwd_GUI(self, setup):
        ReadConfig_ACS_Tree.setSSID(3)
        Packet_Analysis_Result, acs_set_value = Packet_read_ssidPwd.check_packet()
        ...
        if acs_set_value == 'PASS':
            ...
            adb_result = commonmethod.connectWifiAndroid11(ssid=GUI_SSID, password=Input_File.SSID_Pwd_AP3[0],
                                                           security='wpa2', r=0)
            print('adb result' + adb_result)

            if GUI_Pwd == Input_File.SSID_Pwd_AP3[0] and Packet_Analysis_Result == 'PASS' and adb_result == 'Connected':
                result = 'PASS'
            else:
                result = 'FAIL'
        else:
