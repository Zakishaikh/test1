...
@pytest.mark.order(7)
class Test_TR069_ChangeSsidPwdAp1:
    ...
    def test_check_SSIDPwd_GUI(self, setup):
        ...
        if acs_set_value == 'PASS':
            ...
            adb_result = commonmethod.connectWifiAndroid11(ssid=SSID_name, password=Input_File.SSID_Pwd_AP1[0],
                                                           security='wpa2', r=0)
            print('adb result' + adb_result)

            if SSID_pwd == Input_File.SSID_Pwd_AP1[
                0] and Packet_Analysis_Result == 'PASS' and adb_result == 'Connected':
                result = 'PASS'
            else:
                result = 'FAIL'
        else:
